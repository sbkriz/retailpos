...
const logger = LoggerFactory.getInstance().createLogger('SquarespaceTaxStrategy');

/**
 * Squarespace Tax Strategy
 *
 * Zero-rate platform (temporary) - Squarespace has no Tax Rates API.
 * Tax is location-based and calculated server-side only.
 * Spec: section 9.5, 9.6 - Returns zero for all tax codes
 */
export class SquarespaceTaxStrategy extends BaseTaxStrategy {
  constructor() {
    super(ECommercePlatform.SQUARESPACE);
  }
   ...
   *
   * TODO: Replace with proper Squarespace tax resolution strategy once available
   */
  async resolveTax(taxCode?: string): Promise<ResolvedTaxDetail> {
    ...
    if (normalised?.type === 'exempt') {
      logger.debug({ message: 'Squarespace product is exempt', taxCode });
      return {
        rate: 0,
        type: 'exempt',
        name: 'Exempt',
    ...
    logger.debug({ message: 'Squarespace tax rate not available, returning zero', taxCode });
    return {
      rate: 0,
      type: 'exempt',
